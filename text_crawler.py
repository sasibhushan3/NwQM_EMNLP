...
# PATH = "./wikicrawl"
# count = 0
# empty_file = open("empty_users_list.txt", "w")
...
# 	if "main_edits" in os.listdir(user_folder):
# 		files = os.listdir(os.path.join(user_folder, "main_edits"))
# 		if len(files) == 0 :
# 			empty_file.write(user + '\n')
# 			count+=1
# 		else:
# 			sizes.append((len(files), user))
# 			edits = {}
...
# 						continue
# 					try:
# 						tree = ET.parse(os.path.join(os.path.join(user_folder, "main_edits"), xmlfile))
# 					except:
# 						continue
# 					root = tree.getroot()
...
# 					content = {}
# 					content["timestamp"] = root.find("./timestamp").text
# 					content["context"] = ""
# 					for item in root.findall('./prev-context/line'):
# 					    if(item is not None):
# 					        temp_text = item.text
# 					        temp_text = transform(temp_text)

# 					        temp_text = wiki2text(temp_text)
# 					        temp_text = compact(clean(temp_text))
# 					        content["context"] += "".join(temp_text) + "\n" # transform
# 					content["del-line"] = ""
# 					for item in root.findall('./diff-deleted/diff-deletedline'):
# 					    if(item is not None ):
# 					        temp_text = item.text
# 					        temp_text = transform(temp_text)
...
# 					        temp_text = compact(clean(temp_text))
# 					        # print(temp_text)
# 					        content["del-line"] += "".join(temp_text) + "\n" # transform
# 					content["exact-del"] = ""
# 					for item in root.findall('./diff-deleted/diff-changeinline'):
# 					    if(item is not None ):
# 					        temp_text = item.text
# 					        temp_text = transform(temp_text)
...
# 					        temp_text = compact(clean(temp_text))
# 					        # print(temp_text)
# 					        content["exact-del"] += "".join(temp_text) + "\n" # transform
# 					content["add-line"] = ""
# 					for item in root.findall('./diff-added/diff-addedline'):
# 					    if(item is not None ):
# 					        temp_text = item.text
# 					        temp_text = transform(temp_text)
...
# 					        temp_text = compact(clean(temp_text))
# 					        # print(temp_text)
# 					        content["add-line"] += "".join(temp_text) + "\n" # transform
# 					content["exact-add"] = ""
# 					for item in root.findall('./diff-added/diff-changeinline'):
# 					    if(item is not None ):
# 					        temp_text = item.text
# 					        temp_text = transform(temp_text)
...
# 					        temp_text = compact(clean(temp_text))
# 					        # print(temp_text)
# 					        content["exact-add"] += "".join(temp_text) + "\n" # transform
					
# 					if(content):
# 						if(title in edits):
# 							edits[title].append(content)
# 						else:
# 							edits[title] = [content]
# 				except:
# 					print(user_folder, xmlfile)
# 			temp_file = open(os.path.join(user_folder, "cleaned_edits.json"), "w")
# 			if edits:
# 				json.dump(edits, temp_file)
# 			temp_file.close()
# 			# print(len(files))

# print(sorted(sizes, key = lambda x: x[0])[0:10])
# empty_file.close()
# print(count)