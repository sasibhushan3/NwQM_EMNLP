''' This code implements the InceptionV3 model
 
    We use the images (screenshot) of each page and the image is scaled.
    ...
    classification to classify into 6 wikipedia classes.
...
parser = argparse.ArgumentParser(description='Read Arguments for InceptionV3 model')
...
parser.add_argument('--num_epoch', type=int, nargs='?', default=20,
                                        help='Number of epochs for InceptionV3 model')
...
parser.add_argument('--learning_rate', type=float, nargs='?', default=0.0001,
                                        help='Learning rate for InceptionV3 model')
...
# Divide into train, val and test data
train = proc[:20000]
...
test_generator = get_features_label(test, batch_size=args.batch_size)
...
test_steps = int(len(test)/args.batch_size)
...
predIdxs = inception.predict_generator(test_generator, steps=test_steps)
...
print('Accuracy obtained using InceptionV3 model is : ',round(c*100/len(l1),2))
